...
def app():
    st.title("This is GMC Simulation")
    meu = st.number_input("Enter mean of Arrival:",step=1.,format="%.2f")
    lembda=st.number_input("Enter mean of Service:",step=1.,format="%.2f")
    sigma=st.number_input("Enter Variance:",step=1.,format="%.2f")
    server_no=int(st.number_input("Enter Total No of Server:",step=1.,format="%.2f"))
    if  st.button("Generate Plot"):
        df,s_no,arrival,service,WT,RT,TA,server_info,avg_interarrival,avg_service,avg_TA,avg_WT,avg_RT,Servers=GMC(lembda,meu,sigma,server_no)
        st.write(df)
